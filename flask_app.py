...
def format_list_response(response: Union[str, list]) -> str:
    ...
    if isinstance(response, list):
        ...
        try:
            items = ast.literal_eval(response.strip())
...
def ask():
    ...
    try:
        ...
        output = resp.get('output', 'No response from agent')
        formatted = format_list_response(output)
        return jsonify({"output": formatted})
    except Exception as e:
        return jsonify({"output": f"Error processing your request: {e}"}), 500
